...
extern "C" {
    ...
    fn ffi_create_pdf_from_raw_data(json_data: *const c_char, save_path: *const c_char) -> ApplicationError;
}
...
pub fn create_tables_pdf(storage: &Storage, save_path: PathBuf) -> Result<ApplicationError, ()> {
    ...
    // Call function and retrieve Result
    unsafe {
        let error_code = ffi_create_pdf_from_raw_data(serialized_data_cstring_ptr, save_path_cstring_ptr);
        return Ok(error_code);
    }
